...
describe('game store persistence', () => {
  ...
  it('keeps valid world snapshots and removes malformed or unknown-job entries during migration', () => {
    ...
    localStorage.setItem('yuliang-save-v1', JSON.stringify({ ...state, version: 5, worldHistory: [
      { year: 1, day: 337, netWorth: 12000, businessCount: 1, listedBusinessCount: 1, publicFloatPercent: 35, relationshipCount: 2, visitedLocationCount: 3, locationDevelopment: { 'location.central': 2, 'location.unknown': 4, 'location.riverside': 9 }, currentJobId: 'job.seed-shop-clerk' },
    ...
    expect(restored.worldHistory).toEqual([{ year: 1, day: 337, netWorth: 12000, businessCount: 1, listedBusinessCount: 1, publicFloatPercent: 35, relationshipCount: 2, visitedLocationCount: 3, locationDevelopment: { 'location.central': 2 }, currentJobId: 'job.seed-shop-clerk' }]);
  });
