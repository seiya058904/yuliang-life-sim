...
import { housingMortgageTerms, housingPrice, housingRentPerDay } from './game/engine/locations';
    ...
    <div className={`app-shell mode-${shellMode} view-${activeView}`}>
      ...
      <main className="main-content">
        ...
        {activeView === 'wealth' && <PageSections title="财富" description="先看现金与持有，再决定下一笔配置。估值变化与现金收入分开记录。" sections={[
          ...
          { id: 'business', label: '经营', content: <>{Object.keys(game.businesses).length === 0 && <p className="muted">尚未持有企业。可以在「市场」查看入股和合伙机会。</p>}<BusinessGroupView game={game} dispatch={dispatch} /><BusinessOperationsView game={game} dispatch={dispatch} /><BusinessPublicFloatView game={game} dispatch={dispatch} /><BusinessLocationSummary game={game} dispatch={dispatch} /></> },
          { id: 'history', label: '历史', content: <><PortfolioHistory game={game} /><VehicleMaintenanceHistory game={game} /></> },
        ]} />}
...
function CareerToolsDrawer({ game, dispatch, onClose }: { game: GameState; dispatch: (action: GameAction) => void; onClose: () => void }) {
  return <PixelDialog onDismiss={onClose} className="career-tools-drawer" role="dialog" aria-modal="true" aria-label="职业工具">
...
function ForecastPanel({ game, scrollTarget = '.life-planning-section' }: { game: GameState; scrollTarget?: string }) {
  ...
  return <section className="forecast-strip inverse">
    <header className="forecast-head"><h2>本周剩余安排</h2><details className="forecast-scope"><summary>预测范围</summary><small>确定性计划变化 · 不包含随机事件、市场价格变化、未确定招聘结果</small></details></header>
    ...
    <div className="forecast-attrs"><span className="forecast-sub">当前属性 / 预计变化</span>
      {attributes.map(([key, value]) => <div className="forecast-attr" key={key}><PixelIcon name={forecastAttributeIcons[key] ?? 'spark'} size={13} /><span>{attributeLabels[key] ?? key}</span><SegmentMeter label={`${attributeLabels[key]}当前值`} value={getAttribute(game, key)} max={100} segments={10} /><span className="forecast-current">{getAttribute(game, key)}</span><b aria-label={`${attributeLabels[key]}预计变化 ${value}`}>{value > 0 ? '+' : value < 0 ? '-' : '±'}{Math.abs(value)}</b></div>)}
    </div>
    <button className="forecast-more" onClick={() => document.querySelector(scrollTarget)?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' })}><PixelAction label="查看本周计划" /></button>
  </section>;
}
  ...
  return <section className="time-console" aria-label="世界时间">
    <div className="hero-cols">
      ...
      <div className="hero-controls">
        ...
        <button className="text-button" onClick={() => document.querySelector('.inbox-grid')?.scrollIntoView({ block: 'start' })}>查看待处理事项</button>
      </div>
    </div>
  </section>;
}
...
function ShopView({ game, dispatch, onNavigate, initialTab, initialActivityId }: { game: GameState; dispatch: (action: GameAction) => boolean; onNavigate: (view: ViewId) => void; initialTab: string; initialActivityId?: ContentId }) {
  ...
  useEffect(() => {
    ...
    setActivityPage(index < 0 ? 0 : Math.floor(index / 6));
    if (index >= 0) setSelectedKey(`act:${entries[index].activity.id}|${entries[index].option.id}`);
  }, [initialTab, initialActivityId]);
  ...
  const activityPageCount = Math.max(1, Math.ceil(activityEntries.length / 6));
  const safeActivityPage = Math.min(activityPage, activityPageCount - 1);
  const pagedActivityEntries = activityEntries.slice(safeActivityPage * 6, safeActivityPage * 6 + 6);
  ...
  const itemPageCount = Math.max(1, Math.ceil(items.length / 6));
  const featuredItems = items.slice(itemPage * 6, (itemPage + 1) * 6);
  const selectItemPage = (page: number) => {
    setItemPage(page);
    const first = items[page * 6];
      ...
      <aside className="shop-rail" aria-label="商店辅助信息">
        <section className="rail-module rail-cart" aria-label="购物清单"><header><PixelIcon name="bag" size={16} /><h3>购物袋（{cartCount}）</h3></header>
          {cartCount === 0 ? <ShopRailEmptyState illustration="bag" title="购物袋是空的" hint="选择商品后，会在这里结算。" /> : <>
            <ul className="rail-rows">{Object.entries(cart).map(([itemId, quantity]) => { const item = contentRegistry.items.find((entry) => entry.id === itemId); return <li key={itemId}><PixelIllustration name={item ? itemIllustrationFor(item) : 'bag'} size={18} className="rail-row-art" aria-hidden="true" /><span>{item?.name}</span><b>×{quantity}</b><button className="text-button" aria-label={`移除购物袋商品：${item?.name}`} onClick={() => setCart(current => { const next = { ...current }; delete next[itemId]; return next; })}>移除</button></li>; })}</ul>
  ...
  return <section className="social-workspace" aria-label="社交">
    ...
    <div className="contact-workspace">
      <nav className="contact-list" aria-label="联系人">{contentRegistry.characters.map((character, index) => {
        ...
        return <button className="contact-row" key={character.id} aria-pressed={selectedId === character.id} onClick={() => setSelectedId(character.id)}>
          <PixelIllustration name={socialPortraitFor(index)} size={40} /><span><strong>{character.name}</strong><small>{character.identity}</small></span><span className="contact-value"><b>{value}</b><small>{stage?.label ?? '认识'}</small></span>
        </button>;
      })}</nav>
      <div className="contact-detail"><SocialDetail game={game} dispatch={dispatch} characterId={selectedId} /><GiftPanel game={game} dispatch={dispatch} characterId={selectedId} /></div>
    </div>
    <MessageInbox game={game} dispatch={dispatch} />
    <details className="social-stories" open><summary>正在发生的故事</summary><StorylinePanel game={game} dispatch={dispatch} /></details>
    <details><summary>全部人物偏好</summary><CharacterPreferenceSummary game={game} /></details>
  </section>;
}

function SocialDetail({ game, dispatch, characterId }: { game: GameState; dispatch: (action: GameAction) => void; characterId?: ContentId }) {
  const character = contentRegistry.characters.find(entry => entry.id === characterId);
...
function GiftPanel({ game, dispatch, characterId }: { game: GameState; dispatch: (action: GameAction) => void; characterId?: ContentId }) {
  const gifts = contentRegistry.items.filter((item) => item.giftable && (game.inventory[item.id] ?? 0) > 0);
  if (!gifts.length) return <p className="muted">库存中暂无可赠送的礼物。拥有可赠送商品后，可在这里送给已认识的联系人。</p>;
  return <section className="detail-panel" aria-label="礼物"><div className="section-heading compact"><div><span className="eyebrow">小心意，不是关系任务</span><h2>准备一份礼物</h2></div><p>合适的礼物会带来一点额外关系进展；不需要靠昂贵礼物维持关系。</p></div><div className="item-list">{contentRegistry.characters.filter((character) => (!characterId || character.id === characterId) && (game.relationships[character.id] ?? 0) > 0).map((character) => <div className="item-row" key={character.id}><div><h3>{character.name}</h3><span className="muted">偏好：{character.preferredGiftTags?.join('、') ?? '没有特别偏好'}</span></div><div className="button-pair">{gifts.map((item) => <button className="text-button" key={`${character.id}.${item.id}`} onClick={() => dispatch({ type: 'gift_item', characterId: character.id, itemId: item.id })}>送 {item.name}（×{game.inventory[item.id]}）</button>)}</div></div>)}</div></section>;
}
...
function RewardModal({ reward, dispatch }: { reward: NonNullable<GameState['pendingReward']>; dispatch: (action: GameAction) => void }) {
  return <div className="modal-backdrop event-paused"><PixelDialog className="event-modal reward-modal" role="dialog" aria-modal="true" aria-labelledby="reward-title"><span className="eyebrow">本次获得</span><h2 id="reward-title">结果已经写入人生</h2><div className="reward-lines">{reward.lines.map((line, index) => <div key={`${line}-${index}`}>{line}</div>)}</div><p className="reward-auto-note">奖励已经结算。选择接下来是否继续运行。</p><div className="button-pair"><button className="secondary-button" onClick={() => dispatch({ type: 'claim_reward' })}>收下并暂停</button><button className="primary-button" onClick={() => dispatch({ type: 'claim_reward', resume: true })}>收下并继续运行</button></div></PixelDialog></div>;
}
...
function MonthlySummaryModal({ game, dispatch }: { game: GameState; dispatch: (action: GameAction) => void }) {
  ...
  return <div className="modal-backdrop settlement-mode"><PixelModalFrame className="monthly-summary fullframe pixel-corners" role="dialog" aria-modal="true" aria-labelledby="monthly-title">
...
function ConfirmReset({ onCancel, onConfirm }: { onCancel: () => void; onConfirm: () => void }) { return <div className="modal-backdrop"><PixelDialog onDismiss={onCancel} className="confirm-modal" role="dialog" aria-modal="true" aria-label="重新开始确认"><h2>重新开始？</h2><p>当前存档会被新的开始替换，之后可以从头体验。</p><div className="button-pair"><button className="secondary-button" onClick={onCancel}>先不重来</button><button className="primary-button" onClick={onConfirm}>确认重新开始</button></div></PixelDialog></div>; }
