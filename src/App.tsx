...
function WeekPlanner({ game, dispatch }: { game: GameState; dispatch: (action: GameAction) => void }) {
  ...
  const cycle = (weekday: Weekday, slot: PlanSlot) => {
    ...
    const plannedCost = ([1, 2, 3, 4, 5, 6, 7] as const).flatMap((weekday) => [game.weeklyPlan.days[weekday].day, game.weeklyPlan.days[weekday].evening]).reduce((sum, activity) => sum + (activity.kind === 'activity' ? contentRegistry.activities?.flatMap((entry) => entry.options).find((option) => option.id === activity.optionId)?.cashCost ?? 0 : activity.kind === 'course' ? contentRegistry.courses?.find((course) => course.id === activity.courseId)?.cashCost ?? 0 : 0), 0);
